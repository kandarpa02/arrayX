...
@custom_grad
def _max_pool1d(input: torch.Tensor, *, kernel_size, stride=None, padding, dilation, ceil_mode=False):
    ...
    return output, (input,), backward


@custom_grad
def _max_pool2d(input: torch.Tensor, *, kernel_size, stride=None, padding, dilation, ceil_mode=False):
    stride = stride or kernel_size
...
@custom_grad
def _max_pool3d(input: torch.Tensor, *, kernel_size, stride=None, padding, dilation, ceil_mode=False):
    stride = stride or kernel_size
...
@custom_grad
def _avg_pool1d(input: torch.Tensor, *, kernel_size, stride=None, padding, ceil_mode=False, count_include_pad=True):
    stride = stride or kernel_size
    output = F.avg_pool1d(
        input, kernel_size=kernel_size, stride=stride,
...
@custom_grad
def _avg_pool2d(input: torch.Tensor, *, kernel_size, stride=None, padding, ceil_mode=False, count_include_pad=True):
    stride = stride or kernel_size
    output = F.avg_pool2d(
        input, kernel_size=kernel_size, stride=stride,
        padding=padding, ceil_mode=ceil_mode,
...
@custom_grad
def _avg_pool3d(input: torch.Tensor, *, kernel_size, stride=None, padding, ceil_mode=False, count_include_pad=True):
    stride = stride or kernel_size
    output = F.avg_pool3d(
        input, kernel_size=kernel_size, stride=stride,
        padding=padding, ceil_mode=ceil_mode,
...
def max_pool1d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False):
    kernel_size = (kernel_size,) if isinstance(kernel_size, int) else kernel_size
    stride = (stride,) if isinstance(stride, int) else stride
    padding = (padding,) if isinstance(padding, int) else padding
    dilation = (dilation,) if isinstance(dilation, int) else dilation

    return _max_pool1d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, dilation=dilation, ceil_mode=ceil_mode)


def max_pool2d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False):
    kernel_size = (kernel_size, kernel_size) if isinstance(kernel_size, int) else kernel_size
    stride = (stride, stride) if isinstance(stride, int) else stride
    padding = (padding, padding) if isinstance(padding, int) else padding
    dilation = (dilation, dilation) if isinstance(dilation, int) else dilation

    return _max_pool2d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, dilation=dilation, ceil_mode=ceil_mode)


def max_pool3d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False):
    ...
    return _max_pool3d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, dilation=dilation, ceil_mode=ceil_mode)


def avg_pool1d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True):
    kernel_size = (kernel_size,) if isinstance(kernel_size, int) else kernel_size
    stride = (stride,) if isinstance(stride, int) else stride
    padding = (padding,) if isinstance(padding, int) else padding

    return _avg_pool1d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, ceil_mode=ceil_mode, count_include_pad=count_include_pad)


def avg_pool2d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True):
    kernel_size = (kernel_size, kernel_size) if isinstance(kernel_size, int) else kernel_size
    stride = (stride, stride) if isinstance(stride, int) else stride
    padding = (padding, padding) if isinstance(padding, int) else padding

    return _avg_pool2d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, ceil_mode=ceil_mode, count_include_pad=count_include_pad)


def avg_pool3d(input: torch.Tensor, *, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True):
    kernel_size = (kernel_size,) * 3 if isinstance(kernel_size, int) else kernel_size
    stride = (stride,) * 3 if isinstance(stride, int) else stride
    padding = (padding,) * 3 if isinstance(padding, int) else padding

    return _avg_pool3d(input, kernel_size=kernel_size, stride=stride or kernel_size,
                       padding=padding, ceil_mode=ceil_mode, count_include_pad=count_include_pad)
