...
def _leaky_relu(x, negative_slope: float = 0.01):
    out = x.unary_op(lambda t: torch.ops.aten.leaky_relu(t, negative_slope))
...
def _elu(x, alpha: float = 1.0):
    out = x.unary_op(lambda t: torch.ops.aten.elu(t, alpha))
...
def _gelu(x, approximate: str = "none"):
    out = x.unary_op(lambda t: torch.ops.aten.gelu(t, approximate=approximate))
...
@custom_grad
def _softsign(x):
    out = x.unary_op(lambda t: t / (1 + torch.ops.aten.abs(t)))

    def backward(grad, x=x.data):
        denom = 1 + torch.ops.aten.abs(x)
